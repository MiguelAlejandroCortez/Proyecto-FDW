...
class CAD
{
    ...
    static public function agregaUsuario($nombre, $contraseña, $correo, $monto) //funcional se llama en registroCliente
    {
        $con = new Conexion();
        $query = $con->conectar()->prepare("INSERT INTO usuario (nombre, correoe, contraseña, monto) VALUES ('$nombre', '$correo', '$contraseña', '$monto')");
    ...
    {
        $con = new Conexion();
        $queryVendedor = $con->conectar()->prepare("SELECT COUNT(*) FROM vendedor WHERE correoe = '$correo'");
        $queryUsuario = $con->conectar()->prepare("SELECT COUNT(*) FROM usuario WHERE correoe = '$correo'");
        $queryAdmin = $con->conectar()->prepare("SELECT COUNT(*) FROM administrador WHERE correoe = '$correo'");


        if($queryVendedor->execute() && $queryUsuario->execute() && $queryAdmin->execute()){
            $countVendedor = $queryVendedor->fetchColumn();
            $countUsuario = $queryUsuario->fetchColumn();
            $countAdmin = $queryAdmin->fetchColumn();

            if($countVendedor > 0 || $countUsuario > 0 || $countAdmin > 0){
